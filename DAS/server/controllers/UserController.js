                        ...
                        UserType.findOne({ title: req.body[2] }, (err, dataTitle) => {
                            ...
                            if (dataTitle == null || dataTitle == undefined) {
                                ...
                                let school = new School({
                                    schoolId: req.body[0].schoolId,
                                    name: req.body[0].name,
                                    email: req.body[0].email,
                                    address: req.body[0].address,
                                    city: req.body[0].city,
                                    country: req.body[0].country,
                                    contactNo: req.body[0].contactNo,
                                    slogan: req.body[0].slogan,
            ...
            if (account.password == req.body.password) {
                UserType.findOne({ _id: account.userId }, (err, data) => {
                    if (err) {
                        ...
                        let token = jwt.sign({ id: account.email }, 'private.key');
                        res.status(200).json({ 'loginStatus': 'authorized', 'token': token, 'username': account.email, 'type': data.title });
                    }
                })
            }
            else {
